...
def side_bar(df) :
  ...
  choice = s_bar.selectbox('지역 선택(재검색시 상세 검색을 지워 주세요)', area, index = 10, on_change=clear_search(last_choice))
  ...
  if last_choice.get('choice') != choice:
    last_choice['choice'] = choice

  search = s_bar.text_input('상세 검색 (시, 교명등의 키워드를 입력 :smile:)', value=last_choice.get('search', ""), key='search')

  if last_values.get('search') != search:
    last_values['search'] = search
